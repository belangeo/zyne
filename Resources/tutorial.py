...
- In the preferences panel:
    3 - Path to the directory where you saved your python files. 
        Several modules can be defined in a single file.
...
Attributes :
    ...
    self.panL, self.panR : PyoObject
        ...
        has to multiply his left and right signals with these variables in order to use 
        the "Pan" slider.
    self.trig : PyoObject 
...
The "BaseSynth" class is where are handled "pitch", "amplitude", "polyphony", user-defined 
attributes (p1, p2, p3) and samples exportation.
...
self.panL, self.panR : These variables contains panning values for left and right channels. The user
                       has to multiply his left and right signals with these variables in order to 
                       use the "Pan" slider.
...
self.out : This variable must be the object that send the sound to the output. Although it is 
           possible, the custom class should not called the `out` method of any object. Every 
           signals must be mixed in the self.out variable, which will then be sent to the
           post-processing effects and finally to the soundcard.
...
The module documentation, accessible via the question mark on the interface must be included 
in the __doc__ string variable of the custom class. The following syntax must be respected:
...
Example of a file containing two modules. First, a simple module implementing a chorus of 
sine waves using semitone transposition and second, a soundfile looper/slicer using 
transposition factor derived from the keyboard's pitches.
...
class ChoruSyn(BaseSynth):
    ...
    def __init__(self, config):
        ...
        self.pitchVar = Randi(min=0.-self.p3, max=self.p3, 
                              freq=self.p2*[random.uniform(.95, 1.05) for i in range(6)],
                              add=1)
        ...
        self.all = [self.osc1, self.osc2, self.osc3, self.osc4, self.osc5, self.osc6]
        self.out = Mix(self.all, voices=2).out()
...
class SndLooper(BaseSynth):
    ...
    def __init__(self, config):
        ...
        self.looper1 = Looper(self.table, pitch=self.pitch, start=self.st*self.pitchVar[0], 
                              dur=self.dur, interp=4, autosmooth=True,
                              mul=self.amp*self.panL).mix(1)
        self.looper2 = Looper(self.table, pitch=self.pitch, start=self.st*self.pitchVar[1], 
                              dur=self.dur, interp=4, autosmooth=True, 
                              mul=self.amp*self.panR).mix(1)
...
class GenericModule(BaseSynth):
    ...
    def __init__(self, config):
        ...
        BaseSynth.__init__(self, config, mode=1)
        self.fm1 = FM(carrier=self.pitch, ratio=self.p1, index=self.p2,
                      mul=self.amp*self.panL).mix(1)
        self.fm2 = FM(carrier=self.pitch*0.997, ratio=self.p1, index=self.p2, 
                      mul=self.amp*self.panR).mix(1)
